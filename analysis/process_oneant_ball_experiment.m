...
daydir = '/Users/florencet/Documents/matlab_root/chrap-ball/meeting_and_analysis/data/ant-rm/2016-02-16';
...
for aa = 1:length(expfiles)
    ...
    summary_data.vsum_short = nan(creps, 3000);
    summary_data.vsum_long = nan(creps, 3000);
    
    summary_data.vfwd_short = nan(creps, 3000);
    summary_data.vfwd_long = nan(creps, 3000);
    summary_data.vfwd_right = nan(creps, 3000);
    
    summary_dat.om_short = nan(creps, 3000);
    summary_data.om_long = nan(creps, 3000);
    
    summary_data.vss_short = nan(creps, 3000);
    summary_data.vss_long = nan(creps, 3000);
    
    %% do a little bit of identical analysis on every trial
    ...
    for bb = 1:length(sfiles)
        
        load(sfiles(bb).name)
        
        summary_data.vfwd_short(bb, :) = expr.c_trial.data.vfwd;
        summary_data.vsum_short(bb, :) = expr.c_trial.data.vSum;
        summary_data.om_short(bb,:) = expr.c_trial.data.om;
        summary_data.vss_short(bb,:) = expr.c_trial.data.vss;
    ...
    for bb = 1:length(lfiles)
        ...
        summary_data.vfwd_long(bb, :) = expr.c_trial.data.vfwd;
        summary_data.vsum_long(bb, :) = expr.c_trial.data.vSum;
        summary_data.om_long(bb,:) = expr.c_trial.data.om;
        summary_data.vss_long(bb,:) = expr.c_trial.data.vss;
